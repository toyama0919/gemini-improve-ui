...
`,i===0&&(t+="| "+a.map(()=>"---").join(" | ")+` |
`)}),t.trim()}function jt(e,t){const n=t.querySelectorAll("td, th");return"| "+Array.from(n).map(i=>i.textContent?.trim()??"").join(" | ")+" |"}function Wt(e){return e.textContent?.trim()??""}const F=[{id:"default",prompt:"これについて詳しく"}],C=Math.random().toString(36).substr(2,9);function Ft(e){requestAnimationFrame(()=>{requestAnimationFrame(()=>{e.focus();const t=document.createRange(),n=window.getSelection();t.selectNodeContents(e),t.collapse(!1),n?.removeAllRanges(),n?.addRange(t),e.dispatchEvent(new Event("input",{bubbles:!0}))})})}function $e(e,t){const n=document.querySelector('div[contenteditable="true"][role="textbox"]');if(n){for(;n.firstChild;)n.removeChild(n.firstChild);e.split(`
`).forEach(o=>{const i=document.createElement("p");o.trim()===""?i.appendChild(document.createElement("br")):i.textContent=o,n.appendChild(i)}),Ft(n),t&&setTimeout(()=>{const o=document.querySelector('button[aria-label*="送信"], button[aria-label*="Send"]');o&&!o.disabled&&o.click()},100)}}function Vt(e,t){const i=e.getContent().split(`
`).map(r=>`> ${r}`).join(`
`)+`
...
`)})})):(t.querySelectorAll("table[data-path-to-node]").forEach(c=>{const l=He(c);if(l){const u=l.querySelector(".deep-dive-button-inline");if(u){if(u.getAttribute("data-initialized")===C)return;u.remove()}l.querySelectorAll(".deep-dive-expand-button, .deep-dive-child-button").forEach(d=>d.remove()),n.push({type:"table",element:l,getContent:()=>_e(c)})}}),t.querySelectorAll("blockquote[data-path-to-node]").forEach(c=>{const l=c.querySelector(".deep-dive-button-inline");if(l){if(l.getAttribute("data-initialized")===C)return;l.remove()}n.push({type:"blockquote",element:c,getContent:()=>c.textContent?.trim()??""})}),t.querySelectorAll("ol[data-path-to-node], ul[data-path-to-node]").forEach(c=>{const l=c.querySelector(":scope > .deep-dive-button-inline");if(l){if(l.getAttribute("data-initialized")===C)return;c.querySelectorAll(".deep-dive-button-inline, .deep-dive-expand-button").forEach(p=>p.remove())}let u=c.parentElement,d=!1;for(;u&&u!==t;){if((u.tagName==="OL"||u.tagName==="UL")&&u.hasAttribute("data-path-to-node")){d=!0;break}u=u.parentElement}d||n.push({type:"list",element:c,getContent:()=>Wt(c)})})),n.forEach(r=>Gt(r))})}function Ke(){const e=document.getElementById("gemini-deep-dive-mode-selector");e&&e.remove(),chrome.storage.sync.get(["deepDiveModes","currentDeepDiveModeId"],t=>{const n=t.deepDiveModes&&t.deepDiveModes.length>0?t.deepDiveModes:F,o=document.createElement("div");o.id="gemini-deep-dive-mode-selector",o.className="gemini-deep-dive-mode-selector";const i=document.createElement("select");i.id="gemini-deep-dive-mode",i.title="深掘りモード",i.setAttribute("aria-label","深掘りモード"),n.forEach(d=>{const p=document.createElement("option");p.value=d.id,p.textContent=d.id,i.appendChild(p)}),i.addEventListener("change",()=>{chrome.storage.sync.set({currentDeepDiveModeId:i.value})}),o.appendChild(i);const r=document.querySelector('button[aria-label*="ファイル"], button[aria-label*="追加"]'),s=document.querySelector('button[aria-label*="ツール"], button[aria-label*="Tool"]')||r&&r.nextElementSibling;if(s&&s.parentElement)s.parentElement.insertBefore(o,s.nextSibling);else{const d=document.querySelector('div[contenteditable="true"][role="textbox"]');if(d){const p=d.closest("form")||d.parentElement?.parentElement;p?p.insertBefore(o,p.firstChild):document.body.appendChild(o)}else document.body.appendChild(o)}const l=new URLSearchParams(location.search).get("mode_id");let u=t.currentDeepDiveModeId;l&&n.some(d=>d.id===l)&&(u=l,chrome.storage.sync.set({currentDeepDiveModeId:l})),u&&n.some(d=>d.id===u)?i.value=u:n.length>0&&(i.value=n[0].id)})}const Jt=`
  ...
  `;function Zt(){const e="gemini-deep-dive-styles";if(document.getElementById(e))return;const t=document.createElement("style");t.id=e,t.textContent=Jt,document.head.appendChild(t)}let fe=null;function en(){Zt();const e=()=>{document.querySelector('button[aria-label*="ツール"], button[aria-label*="Tool"], button[aria-label*="ファイル"], button[aria-label*="追加"]')||document.querySelector('div[contenteditable="true"][role="textbox"]')?Ke():setTimeout(e,500)};e(),chrome.storage.onChanged.addListener((n,o)=>{o==="sync"&&n.deepDiveModes&&location.href.includes("gemini.google.com")&&document.querySelector('button[aria-label*="ツール"], button[aria-label*="Tool"], div[contenteditable="true"][role="textbox"]')&&Ke()}),new MutationObserver(n=>{let o=!1;for(const i of n){if(i.addedNodes.length>0){for(const r of i.addedNodes)if(r.nodeType===1){const a=r;if(a.matches?.("[data-path-to-node]")||a.querySelector?.("[data-path-to-node]")){o=!0;break}}}if(o)break}o&&(fe&&clearTimeout(fe),fe=setTimeout(()=>ze(),500))}).observe(document.body,{childList:!0,subtree:!0}),setTimeout(()=>ze(),1e3)}const Q="gemini-improve-ui-map-visible";function tn(e){return`
    #gemini-map-panel {
      position: fixed;
      right: 16px;
      top: 60px;
...
`)}),n}async copyToClipboard(){const t=this.exportForAI();try{return await navigator.clipboard.writeText(t),this.showNotification("ページ構造をクリップボードにコピーしました"),!0}catch{return this.showNotification("コピーに失敗しました","error"),!1}}showNotification(t,n="success"){const o=document.createElement("div");o.style.cssText=`
    ...
    `,document.head.appendChild(i),document.body.appendChild(o),setTimeout(()=>{o.style.transition="opacity 0.3s",o.style.opacity="0",setTimeout(()=>o.remove(),300)},3e3)}}function pn(){window.domAnalyzer=new un,window.analyzePage=()=>{console.log(window.domAnalyzer.capturePageStructure())},window.copyPageStructure=()=>{window.domAnalyzer.copyToClipboard()}}const mn=`
    .gems-list-container {
  ...
  `,fn={matches:["https://gemini.google.com/*"],runAt:"document_end",main(){window.rememberActionButtonPosition=_t,pn(),yn()}};function Ye(){const e="gemini-improve-ui-custom-styles";document.getElementById(e)?.remove();const t=document.createElement("style");t.id=e,t.textContent=mn,document.head.appendChild(t)}function Xe(e){document.documentElement.style.setProperty("--chat-max-width",`${e}px`)}function hn(){chrome.storage.sync.get(["chatWidth"],e=>{Xe(e.chatWidth||900)})}function bn(){setTimeout(()=>{te(),Ae(),document.getElementById("gemini-export-note-button")?.remove(),Me(),K()||(Qe(),Ne())},ne)}function gn(){setTimeout(()=>{document.getElementById("gemini-export-note-button")?.remove(),Me(),Qe()},ne)}function yn(){hn(),Ye(),window.addEventListener("popstate",()=>{se()});let e=location.href;new MutationObserver(()=>{const t=location.href;t!==e&&(e=t,window.rememberActionButtonPosition?.(-1),dn(),bn())}).observe(document,{subtree:!0,childList:!0}),Ut(),K()?(Et(),Ae()):(ht(),en(),Ne(),gn()),chrome.storage.onChanged.addListener((t,n)=>{n==="sync"&&t.chatWidth&&(Xe(t.chatWidth.newValue),Ye())})}function X(e,...t){}const wn={debug:(...e)=>X(console.debug,...e),log:(...e)=>X(console.log,...e),warn:(...e)=>X(console.warn,...e),error:(...e)=>X(console.error,...e)},Je=globalThis.browser?.runtime?.id?globalThis.browser:globalThis.chrome;var Ze=class et extends Event{static EVENT_NAME=he("wxt:locationchange");constructor(t,n){super(et.EVENT_NAME,{}),this.newUrl=t,this.oldUrl=n}};function he(e){return`${Je?.runtime?.id}:content:${e}`}const xn=typeof globalThis.navigation?.addEventListener=="function";function vn(e){let t,n=!1;return{run(){n||(n=!0,t=new URL(location.href),xn?globalThis.navigation.addEventListener("navigate",o=>{const i=new URL(o.destination.url);i.href!==t.href&&(window.dispatchEvent(new Ze(i,t)),t=i)},{signal:e.signal}):e.setInterval(()=>{const o=new URL(location.href);o.href!==t.href&&(window.dispatchEvent(new Ze(o,t)),t=o)},1e3))}}}var En=class ${static SCRIPT_STARTED_MESSAGE_TYPE=he("wxt:content-script-started");id;abortController;locationWatcher=vn(this);constructor(t,n){this.contentScriptName=t,this.options=n,this.id=Math.random().toString(36).slice(2),this.abortController=new AbortController,this.stopOldScripts(),this.listenForNewerScripts()}get signal(){return this.abortController.signal}abort(t){return this.abortController.abort(t)}get isInvalid(){return Je.runtime?.id==null&&this.notifyInvalidated(),this.signal.aborted}get isValid(){return!this.isInvalid}onInvalidated(t){return this.signal.addEventListener("abort",t),()=>this.signal.removeEventListener("abort",t)}block(){return new Promise(()=>{})}setInterval(t,n){const o=setInterval(()=>{this.isValid&&t()},n);return this.onInvalidated(()=>clearInterval(o)),o}setTimeout(t,n){const o=setTimeout(()=>{this.isValid&&t()},n);return this.onInvalidated(()=>clearTimeout(o)),o}requestAnimationFrame(t){const n=requestAnimationFrame((...o)=>{this.isValid&&t(...o)});return this.onInvalidated(()=>cancelAnimationFrame(n)),n}requestIdleCallback(t,n){const o=requestIdleCallback((...i)=>{this.signal.aborted||t(...i)},n);return this.onInvalidated(()=>cancelIdleCallback(o)),o}addEventListener(t,n,o,i){n==="wxt:locationchange"&&this.isValid&&this.locationWatcher.run(),t.addEventListener?.(n.startsWith("wxt:")?he(n):n,o,{...i,signal:this.signal})}notifyInvalidated(){this.abort("Content script context invalidated"),wn.debug(`Content script "${this.contentScriptName}" context invalidated`)}stopOldScripts(){document.dispatchEvent(new CustomEvent($.SCRIPT_STARTED_MESSAGE_TYPE,{detail:{contentScriptName:this.contentScriptName,messageId:this.id}})),window.postMessage({type:$.SCRIPT_STARTED_MESSAGE_TYPE,contentScriptName:this.contentScriptName,messageId:this.id},"*")}verifyScriptStartedEvent(t){const n=t.detail?.contentScriptName===this.contentScriptName,o=t.detail?.messageId===this.id;return n&&!o}listenForNewerScripts(){const t=n=>{!(n instanceof CustomEvent)||!this.verifyScriptStartedEvent(n)||this.notifyInvalidated()};document.addEventListener($.SCRIPT_STARTED_MESSAGE_TYPE,t),this.onInvalidated(()=>document.removeEventListener($.SCRIPT_STARTED_MESSAGE_TYPE,t))}};function Dn(){}function J(e,...t){}const Sn={debug:(...e)=>J(console.debug,...e),log:(...e)=>J(console.log,...e),warn:(...e)=>J(console.warn,...e),error:(...e)=>J(console.error,...e)};return(async()=>{try{const{main:e,...t}=fn;return await e(new En("content",t))}catch(e){throw Sn.error('The content script "content" crashed on startup!',e),e}})()})();
content;