var content=(function(){"use strict";function Xt(e){return e}const se={chat:{navigateToSearch:"Insert",toggleSidebar:"Delete",toggleHistoryMode:"End",scrollUp:"PageUp",scrollDown:"PageDown",historyUp:"ArrowUp",historyDown:"ArrowDown",historyOpen:"Enter",historyExit:"Escape"},search:{moveUp:"ArrowUp",moveDown:"ArrowDown",openResult:"Enter",scrollUp:"PageUp",scrollDown:"PageDown"}};let R=null;function je(){return new Promise(e=>{chrome.storage.sync.get(["shortcuts"],t=>{t.shortcuts?R=t.shortcuts:R=JSON.parse(JSON.stringify(se)),e(R)})})}function ce(){return R||se}function h(e,t){const n=ce(),i=t.split(".");let o=n;for(const r of i)if(o=o[r],!o)return!1;if(typeof o=="object"){const r=o.meta?e.metaKey:!e.metaKey,a=o.ctrl?e.ctrlKey:!e.ctrlKey,s=o.shift?e.shiftKey:!e.shiftKey;return e.code===o.key&&r&&a&&s}return e.code===o&&!e.ctrlKey&&!e.metaKey&&!e.shiftKey}const We=500,Fe=300,le=10,de=40,ue=100;let g=null,f=-1,w=[],G=null;function $(){return g!==null&&g.style.display==="block"&&w.length>0}function v(e){e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation()}function H(e){e==="next"?f=f<0?0:(f+1)%w.length:f=f<0||f<=0?w.length-1:f-1,he()}async function pe(e){if(!e||e.trim().length===0)return[];try{const t=encodeURIComponent(e.trim());return(await(await fetch(`https://www.google.co.jp/complete/search?output=firefox&hl=ja&ie=utf-8&oe=utf-8&q=${t}`)).json())[1]||[]}catch{return[]}}function Ve(){if(g)return g;const e=document.createElement("div");return e.className="gemini-autocomplete-list",e.style.cssText=`
    position: fixed;
    background: white;
...
`;case"thead":case"tbody":case"tr":case"td":case"th":return"";default:return s()}}function i(o){const r=Array.from(o.querySelectorAll("tr"));if(r.length===0)return"";const a=p=>Array.from(p.querySelectorAll("td, th")).map(l=>Array.from(l.childNodes).map(n).join("").replace(/\n+/g," ").trim()),[s,...c]=r,u=a(s),d=u.map(()=>"---");return[`| ${u.join(" | ")} |`,`| ${d.join(" | ")} |`,...c.map(p=>`| ${a(p).join(" | ")} |`)].join(`
...
`).trim()}async function bt(){const e=document.querySelector("infinite-scroller.chat-history");if(!e)return;P("メッセージを読み込み中...");let t=0;for(let n=0;n<30;n++){e.scrollTop=0,await new Promise(o=>setTimeout(o,400));const i=document.querySelectorAll("user-query").length;if(i===t)break;t=i}e.scrollTop=e.scrollHeight}function yt(){const e=Array.from(document.querySelectorAll("user-query")),t=Array.from(document.querySelectorAll("model-response")),n=[],i=Math.min(e.length,t.length);for(let o=0;o<i;o++){const r=Array.from(e[o].querySelectorAll(".query-text-line")).map(u=>u.innerText.trim()).filter(Boolean).join(`
`),a=t[o].querySelector("message-content .markdown"),s=a?ht(a).trim():void 0,c=s?gt(s):"";(r||c)&&n.push({user:r||"",model:c||""})}return n}function ne(){return location.pathname.split("/").pop()||"unknown"}function O(e){return'"'+e.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function Se(e,t){return e.split(`
`).map(n=>n===""?"":t+n).join(`
`)}function xt(e){const t=new Date,n=l=>String(l).padStart(2,"0"),o=`${`${t.getFullYear()}-${n(t.getMonth()+1)}-${n(t.getDate())}`}T${n(t.getHours())}:${n(t.getMinutes())}:${n(t.getSeconds())}`,r=o.replace(/[-:T]/g,""),a=document.querySelector('[data-test-id="conversation-title"]')?.innerText?.trim(),s=(e[0]?.user||"").split(`
`).map(l=>l.trim()).filter(Boolean),c=s.find(l=>!/^https?:\/\//i.test(l))||s[0]||"Gemini chat",u=(a||c).slice(0,60),d=ne(),p=[`id: ${O(d)}`,`title: ${O("Gemini: "+u)}`,`date: ${O(o)}`,`source: ${O(location.href)}`,"tags:","  - gemini","  - fleeting","chats:"];for(const l of e)p.push("  - q: |"),p.push(Se(l.user,"      ")),p.push("    a: |"),p.push(Se(l.model,"      "));return{markdown:p.join(`
`),id:r,title:u}}async function Ae(e=!1){await bt();const t=yt();if(t.length===0){P("保存できる会話が見つかりません","error");return}let n;try{if(e){const c=await window.showDirectoryPicker({mode:"readwrite"});await Ee(c),y=c,n=c,P(`保存先を変更: ${c.name}`)}else n=await mt()}catch{return}const{markdown:i,title:o}=xt(t),r=ne(),s=`gemini-${o.replace(/[\\/:*?"<>|]/g,"").replace(/\s+/g,"-").slice(0,40)}-${r}.yaml`;try{const d=await(await(await n.getDirectoryHandle("inbox",{create:!0})).getFileHandle(s,{create:!0})).createWritable();await d.write(i),await d.close(),P(`保存しました: inbox/${s}`)}catch{P("保存に失敗しました","error")}}function P(e,t="success"){const n=document.getElementById("gemini-export-notification");n&&n.remove();const i=document.createElement("div");i.id="gemini-export-notification",i.style.cssText=`
    position: fixed;
    bottom: 24px;
    right: 24px;
...
Shift+クリックで保存先を変更`,t.textContent="💾 Save note",t.style.cssText=`
  ...
  `,t.addEventListener("mouseenter",()=>{t.style.background="#1557b0"}),t.addEventListener("mouseleave",()=>{t.style.background="#1a73e8"}),t.addEventListener("click",n=>Ae(n.shiftKey)),document.body.appendChild(t)}function Ce(){ne()!=="app"&&wt()}let oe=-1;function vt(e){oe=e}function Et(e){if($()&&(e.key==="ArrowUp"||e.key==="ArrowDown"||e.key==="Enter"||e.key==="Tab"||e.key==="Escape"))return!1;if(h(e,"chat.navigateToSearch"))return e.preventDefault(),xe(),!0;if(h(e,"search.moveUp"))return e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation(),st(),!0;if(h(e,"search.moveDown"))return e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation(),ct(),!0;if(h(e,"search.openResult"))return e.isComposing?!1:(e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation(),lt(),!0);if(h(e,"search.scrollUp"))return e.preventDefault(),window.scrollBy({top:-window.innerHeight*.8,behavior:"auto"}),!0;if(h(e,"search.scrollDown"))return e.preventDefault(),window.scrollBy({top:window.innerHeight*.8,behavior:"auto"}),!0;const t=ce();return!!Object.values(t.chat).includes(e.code)}function St(e){const t=e.target.matches('input, textarea, [contenteditable="true"]');if($()&&(e.key==="ArrowUp"||e.key==="ArrowDown"||e.key==="Enter"||e.key==="Tab"||e.key==="Escape"))return!1;if(e.code==="Home"&&!e.metaKey&&!e.ctrlKey&&!t)return e.preventDefault(),Ae(e.shiftKey),!0;if(e.ctrlKey&&e.shiftKey&&e.code==="KeyD")return e.preventDefault(),window.domAnalyzer?.copyToClipboard(),!0;if(h(e,"chat.navigateToSearch"))return e.preventDefault(),xe(),!0;if(h(e,"chat.toggleSidebar"))return e.preventDefault(),nt(),!0;if(h(e,"chat.toggleHistoryMode")){e.preventDefault();const n=X(),i=n.length>0;if(q())_(),be();else if(t)if(i){let o=oe;(o<0||o>=n.length)&&(o=n.length-1),n[o].focus()}else ye();else{const o=document.activeElement;if(o&&(o.classList?.contains("deep-dive-button-inline")||o.getAttribute("data-action")==="deep-dive")){const a=n.findIndex(s=>s===o);a!==-1&&(oe=a),ye()}else be()}return!0}if(q()&&h(e,"chat.historyExit"))return e.preventDefault(),_(),!0;if(h(e,"chat.scrollUp"))return e.preventDefault(),ge("up"),!0;if(h(e,"chat.scrollDown"))return e.preventDefault(),ge("down"),!0;if(q()){if(h(e,"chat.historyUp"))return e.preventDefault(),it(),!0;if(h(e,"chat.historyDown"))return e.preventDefault(),rt(),!0;if(h(e,"chat.historyOpen"))return e.preventDefault(),at(),!0}if(!q()&&t&&(h(e,"chat.historyUp")||h(e,"chat.historyDown"))){const n=document.querySelector('div[contenteditable="true"][role="textbox"]');if(n&&n.textContent?.trim()===""){e.preventDefault();const i=h(e,"chat.historyUp")?"up":"down";return Ze(i),!0}}if(!q()&&!t){const n=document.activeElement;if(n&&(n.classList?.contains("deep-dive-button-inline")||n.getAttribute("data-action")==="deep-dive")){if(h(e,"chat.historyUp")||h(e,"chat.historyDown")){e.preventDefault();const o=h(e,"chat.historyUp")?"up":"down";return et(o),!0}if(e.key==="ArrowRight"||e.key==="ArrowLeft"){e.preventDefault();const o=n._expandButton,r=n._deepDiveTarget;if(o&&r){const a=o.getAttribute("data-action")==="collapse";(e.key==="ArrowRight"&&!a||e.key==="ArrowLeft"&&a)&&o.click()}return!0}if(h(e,"chat.historyOpen"))return e.preventDefault(),n.click(),!0}}return!1}function At(){je().then(()=>{document.addEventListener("keydown",e=>{if(M()){Et(e);return}St(e)},!0)})}const z=[{id:"default",prompt:"これについて詳しく"}];function De(){const e=document.querySelectorAll(".markdown-main-panel");e.length!==0&&e.forEach(t=>{const n=[],i=t.querySelectorAll("h1[data-path-to-node], h2[data-path-to-node], h3[data-path-to-node], h4[data-path-to-node], h5[data-path-to-node], h6[data-path-to-node]");i.length>0?(i.forEach(a=>{const s=a.querySelector(".deep-dive-button-inline");if(s){if(s.hasAttribute("data-initialized"))return;a.querySelectorAll(".deep-dive-button-inline, .deep-dive-expand-button").forEach(c=>c.remove())}n.push({type:"section",element:a,getContent:()=>Ct(a)})}),t.querySelectorAll("table[data-path-to-node]").forEach(a=>{const s=a.closest(".table-block-component");if(s){const c=s.querySelector(".deep-dive-button-inline");if(c){if(c.hasAttribute("data-initialized"))return;c.remove()}n.push({type:"table",element:s,getContent:()=>Te(a)})}})):(t.querySelectorAll("table[data-path-to-node]").forEach(c=>{const u=c.closest(".table-block-component");if(u){const d=u.querySelector(".deep-dive-button-inline");if(d){if(d.hasAttribute("data-initialized"))return;d.remove()}n.push({type:"table",element:u,getContent:()=>Te(c)})}}),t.querySelectorAll("blockquote[data-path-to-node]").forEach(c=>{const u=c.querySelector(".deep-dive-button-inline");if(u){if(u.hasAttribute("data-initialized"))return;u.remove()}n.push({type:"blockquote",element:c,getContent:()=>c.textContent?.trim()??""})}),t.querySelectorAll("ol[data-path-to-node], ul[data-path-to-node]").forEach(c=>{const u=c.querySelector(":scope > .deep-dive-button-inline");if(u){if(u.hasAttribute("data-initialized"))return;c.querySelectorAll(".deep-dive-button-inline, .deep-dive-expand-button").forEach(l=>l.remove())}let d=c.parentElement,p=!1;for(;d&&d!==t;){if((d.tagName==="OL"||d.tagName==="UL")&&d.hasAttribute("data-path-to-node")){p=!0;break}d=d.parentElement}p||n.push({type:"list",element:c,getContent:()=>Dt(c)})})),n.forEach(r=>Tt(r))})}function Ct(e){let t=(e.textContent?.trim()??"")+`
...
`;else{const a=await new Promise(l=>{chrome.storage.sync.get(["deepDiveModes","currentDeepDiveModeId"],l)}),s=a.deepDiveModes&&a.deepDiveModes.length>0?a.deepDiveModes:z;let d=new URLSearchParams(location.search).get("mode_id")||a.currentDeepDiveModeId||s[0]?.id;s.some(l=>l.id===d)||(d=s[0]?.id);const p=s.find(l=>l.id===d)||s[0]||z[0];o=i+`

`+(p.prompt||"これについて詳しく"),r=!0}qe(o,r)}function Pt(){const e="gemini-deep-dive-styles";if(document.getElementById(e))return;const t=document.createElement("style");t.id=e,t.textContent=`
  ...
  `,document.head.appendChild(t)}function Pe(){const e=document.getElementById("gemini-deep-dive-mode-selector");e&&e.remove(),chrome.storage.sync.get(["deepDiveModes","currentDeepDiveModeId"],t=>{const n=t.deepDiveModes&&t.deepDiveModes.length>0?t.deepDiveModes:z,i=document.createElement("div");i.id="gemini-deep-dive-mode-selector",i.className="gemini-deep-dive-mode-selector";const o=document.createElement("select");o.id="gemini-deep-dive-mode",o.title="深掘りモード",o.setAttribute("aria-label","深掘りモード"),n.forEach(p=>{const l=document.createElement("option");l.value=p.id,l.textContent=p.id,o.appendChild(l)}),o.addEventListener("change",()=>{chrome.storage.sync.set({currentDeepDiveModeId:o.value})}),i.appendChild(o);const r=document.querySelector('button[aria-label*="ファイル"], button[aria-label*="追加"]'),s=document.querySelector('button[aria-label*="ツール"], button[aria-label*="Tool"]')||r&&r.nextElementSibling;if(s&&s.parentElement)s.parentElement.insertBefore(i,s.nextSibling);else{const p=document.querySelector('div[contenteditable="true"][role="textbox"]');if(p){const l=p.closest("form")||p.parentElement?.parentElement;l?l.insertBefore(i,l.firstChild):document.body.appendChild(i)}else document.body.appendChild(i)}const u=new URLSearchParams(location.search).get("mode_id");let d=t.currentDeepDiveModeId;u&&n.some(p=>p.id===u)&&(d=u,chrome.storage.sync.set({currentDeepDiveModeId:u})),d&&n.some(p=>p.id===d)?o.value=d:n.length>0&&(o.value=n[0].id)})}let re=null;function Bt(){Pt();const e=()=>{document.querySelector('button[aria-label*="ツール"], button[aria-label*="Tool"], button[aria-label*="ファイル"], button[aria-label*="追加"]')||document.querySelector('div[contenteditable="true"][role="textbox"]')?Pe():setTimeout(e,500)};e(),chrome.storage.onChanged.addListener((n,i)=>{i==="sync"&&n.deepDiveModes&&location.href.includes("gemini.google.com")&&document.querySelector('button[aria-label*="ツール"], button[aria-label*="Tool"], div[contenteditable="true"][role="textbox"]')&&Pe()}),new MutationObserver(n=>{let i=!1;for(const o of n){if(o.addedNodes.length>0){for(const r of o.addedNodes)if(r.nodeType===1){const a=r;if(a.matches?.("[data-path-to-node]")||a.querySelector?.("[data-path-to-node]")){i=!0;break}}}if(i)break}i&&(re&&clearTimeout(re),re=setTimeout(()=>De(),500))}).observe(document.body,{childList:!0,subtree:!0}),setTimeout(()=>De(),1e3)}let j=!1;const T="gemini-map-panel",Be="gemini-map-styles";function Lt(){if(document.getElementById(Be))return;const e=document.createElement("style");e.id=Be,e.textContent=`
    #gemini-map-panel {
      position: fixed;
      right: 16px;
      top: 60px;
...
`)}),n}async copyToClipboard(){const t=this.exportForAI();try{return await navigator.clipboard.writeText(t),this.showNotification("ページ構造をクリップボードにコピーしました"),!0}catch{return this.showNotification("コピーに失敗しました","error"),!1}}showNotification(t,n="success"){const i=document.createElement("div");i.style.cssText=`
    ...
    `,document.head.appendChild(o),document.body.appendChild(i),setTimeout(()=>{i.style.transition="opacity 0.3s",i.style.opacity="0",setTimeout(()=>i.remove(),300)},3e3)}}function zt(){window.domAnalyzer=new Ot,window.analyzePage=()=>{console.log(window.domAnalyzer.capturePageStructure())},window.copyPageStructure=()=>{window.domAnalyzer.copyToClipboard()}}const Kt={matches:["https://gemini.google.com/app*","https://gemini.google.com/search*"],runAt:"document_end",main(){window.rememberActionButtonPosition=vt,zt(),Wt()}};function Ue(){const e="gemini-improve-ui-custom-styles";document.getElementById(e)?.remove();const t=document.createElement("style");t.id=e,t.textContent=`
    .gems-list-container {
  ...
  `,document.head.appendChild(t)}function _e(e){document.documentElement.style.setProperty("--chat-max-width",`${e}px`)}function jt(){chrome.storage.sync.get(["chatWidth"],e=>{_e(e.chatWidth||900)})}function Wt(){jt(),Ue(),window.addEventListener("popstate",()=>{_()});let e=location.href;new MutationObserver(()=>{const t=location.href;t!==e&&(e=t,window.rememberActionButtonPosition?.(-1),_t(),setTimeout(()=>{Q(),fe(),M()||He(),document.getElementById("gemini-export-note-button")?.remove(),Ce()},1500))}).observe(document,{subtree:!0,childList:!0}),At(),M()?(dt(),fe()):(ot(),Bt(),setTimeout(()=>{Ce()},1500),setTimeout(()=>{He()},1500)),chrome.storage.onChanged.addListener((t,n)=>{n==="sync"&&t.chatWidth&&(_e(t.chatWidth.newValue),Ue())})}function W(e,...t){}const Ft={debug:(...e)=>W(console.debug,...e),log:(...e)=>W(console.log,...e),warn:(...e)=>W(console.warn,...e),error:(...e)=>W(console.error,...e)},Oe=globalThis.browser?.runtime?.id?globalThis.browser:globalThis.chrome;var ze=class Ke extends Event{static EVENT_NAME=ae("wxt:locationchange");constructor(t,n){super(Ke.EVENT_NAME,{}),this.newUrl=t,this.oldUrl=n}};function ae(e){return`${Oe?.runtime?.id}:content:${e}`}const Vt=typeof globalThis.navigation?.addEventListener=="function";function Gt(e){let t,n=!1;return{run(){n||(n=!0,t=new URL(location.href),Vt?globalThis.navigation.addEventListener("navigate",i=>{const o=new URL(i.destination.url);o.href!==t.href&&(window.dispatchEvent(new ze(o,t)),t=o)},{signal:e.signal}):e.setInterval(()=>{const i=new URL(location.href);i.href!==t.href&&(window.dispatchEvent(new ze(i,t)),t=i)},1e3))}}}var Yt=class N{static SCRIPT_STARTED_MESSAGE_TYPE=ae("wxt:content-script-started");id;abortController;locationWatcher=Gt(this);constructor(t,n){this.contentScriptName=t,this.options=n,this.id=Math.random().toString(36).slice(2),this.abortController=new AbortController,this.stopOldScripts(),this.listenForNewerScripts()}get signal(){return this.abortController.signal}abort(t){return this.abortController.abort(t)}get isInvalid(){return Oe.runtime?.id==null&&this.notifyInvalidated(),this.signal.aborted}get isValid(){return!this.isInvalid}onInvalidated(t){return this.signal.addEventListener("abort",t),()=>this.signal.removeEventListener("abort",t)}block(){return new Promise(()=>{})}setInterval(t,n){const i=setInterval(()=>{this.isValid&&t()},n);return this.onInvalidated(()=>clearInterval(i)),i}setTimeout(t,n){const i=setTimeout(()=>{this.isValid&&t()},n);return this.onInvalidated(()=>clearTimeout(i)),i}requestAnimationFrame(t){const n=requestAnimationFrame((...i)=>{this.isValid&&t(...i)});return this.onInvalidated(()=>cancelAnimationFrame(n)),n}requestIdleCallback(t,n){const i=requestIdleCallback((...o)=>{this.signal.aborted||t(...o)},n);return this.onInvalidated(()=>cancelIdleCallback(i)),i}addEventListener(t,n,i,o){n==="wxt:locationchange"&&this.isValid&&this.locationWatcher.run(),t.addEventListener?.(n.startsWith("wxt:")?ae(n):n,i,{...o,signal:this.signal})}notifyInvalidated(){this.abort("Content script context invalidated"),Ft.debug(`Content script "${this.contentScriptName}" context invalidated`)}stopOldScripts(){document.dispatchEvent(new CustomEvent(N.SCRIPT_STARTED_MESSAGE_TYPE,{detail:{contentScriptName:this.contentScriptName,messageId:this.id}})),window.postMessage({type:N.SCRIPT_STARTED_MESSAGE_TYPE,contentScriptName:this.contentScriptName,messageId:this.id},"*")}verifyScriptStartedEvent(t){const n=t.detail?.contentScriptName===this.contentScriptName,i=t.detail?.messageId===this.id;return n&&!i}listenForNewerScripts(){const t=n=>{!(n instanceof CustomEvent)||!this.verifyScriptStartedEvent(n)||this.notifyInvalidated()};document.addEventListener(N.SCRIPT_STARTED_MESSAGE_TYPE,t),this.onInvalidated(()=>document.removeEventListener(N.SCRIPT_STARTED_MESSAGE_TYPE,t))}};function Zt(){}function F(e,...t){}const Qt={debug:(...e)=>F(console.debug,...e),log:(...e)=>F(console.log,...e),warn:(...e)=>F(console.warn,...e),error:(...e)=>F(console.error,...e)};return(async()=>{try{const{main:e,...t}=Kt;return await e(new Yt("content",t))}catch(e){throw Qt.error('The content script "content" crashed on startup!',e),e}})()})();
content;