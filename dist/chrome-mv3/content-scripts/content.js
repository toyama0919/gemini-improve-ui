...
`),a=t[i].querySelector("message-content .markdown"),s=a?vt(a).trim():void 0,c=s?St(s):"";(r||c)&&n.push({user:r||"",model:c||""})}return n}function ae(){return location.pathname.split("/").pop()||"unknown"}function j(e){return'"'+e.replace(/\\/g,"\\\\").replace(/"/g,'\\"')+'"'}function Te(e,t){return e.split(`
`).map(n=>n===""?"":t+n).join(`
`)}function Tt(e){const t=new Date,n=p=>String(p).padStart(2,"0"),i=`${`${t.getFullYear()}-${n(t.getMonth()+1)}-${n(t.getDate())}`}T${n(t.getHours())}:${n(t.getMinutes())}:${n(t.getSeconds())}`,r=i.replace(/[-:T]/g,""),a=document.querySelector('[data-test-id="conversation-title"]')?.innerText?.trim(),s=(e[0]?.user||"").split(`
`).map(p=>p.trim()).filter(Boolean),c=s.find(p=>!/^https?:\/\//i.test(p))||s[0]||"Gemini chat",l=(a||c).slice(0,60),u=ae(),d=[`id: ${j(u)}`,`title: ${j("Gemini: "+l)}`,`date: ${j(i)}`,`source: ${j(location.href)}`,"tags:","  - gemini","  - fleeting","chats:"];for(const p of e)d.push("  - q: |"),d.push(Te(p.user,"      ")),d.push("    a: |"),d.push(Te(p.model,"      "));return{markdown:d.join(`
`),id:r,title:l}}async function De(e=!1){await At();const t=Ct();if(t.length===0){R("保存できる会話が見つかりません","error");return}let n;try{if(e){const c=await window.showDirectoryPicker({mode:"readwrite"});await Ce(c),y=c,n=c,R(`保存先を変更: ${c.name}`)}else n=await wt()}catch{return}const{markdown:o,title:i}=Tt(t),r=ae(),s=`gemini-${i.replace(/[\\/:*?"<>|]/g,"").replace(/\s+/g,"-").slice(0,40)}-${r}.yaml`;try{const u=await(await(await n.getDirectoryHandle("inbox",{create:!0})).getFileHandle(s,{create:!0})).createWritable();await u.write(o),await u.close(),R(`保存しました: inbox/${s}`)}catch{R("保存に失敗しました","error")}}function R(e,t="success"){const n=document.getElementById("gemini-export-notification");n&&n.remove();const o=document.createElement("div");o.id="gemini-export-notification",o.style.cssText=`
    position: fixed;
    bottom: 24px;
    right: 24px;
...
`)}),n}async copyToClipboard(){const t=this.exportForAI();try{return await navigator.clipboard.writeText(t),this.showNotification("ページ構造をクリップボードにコピーしました"),!0}catch{return this.showNotification("コピーに失敗しました","error"),!1}}showNotification(t,n="success"){const o=document.createElement("div");o.style.cssText=`
    ...
    `,document.head.appendChild(i),document.body.appendChild(o),setTimeout(()=>{o.style.transition="opacity 0.3s",o.style.opacity="0",setTimeout(()=>o.remove(),300)},3e3)}}function nn(){window.domAnalyzer=new tn,window.analyzePage=()=>{console.log(window.domAnalyzer.capturePageStructure())},window.copyPageStructure=()=>{window.domAnalyzer.copyToClipboard()}}const on={matches:["https://gemini.google.com/app*","https://gemini.google.com/search*"],runAt:"document_end",main(){window.rememberActionButtonPosition=Lt,nn(),an()}};function Fe(){const e="gemini-improve-ui-custom-styles";document.getElementById(e)?.remove();const t=document.createElement("style");t.id=e,t.textContent=`
    .gems-list-container {
